...
pub fn depth_history_route(collection: Collection<DepthHistory>) -> impl Filter<Extract = impl warp::Reply, Error = warp::Rejection> + Clone {
    warp::path!("depth-history")
        .and(warp::get())
        .and(warp::query::<DepthHistoryQueryParams>())
        .and_then({
            let collection = collection.clone();
            move |params: DepthHistoryQueryParams| {
                let collection = collection.clone();
                async move {
